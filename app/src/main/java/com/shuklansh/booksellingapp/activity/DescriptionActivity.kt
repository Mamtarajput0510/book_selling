...
class DescriptionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent != null) {
            bookId =
                intent.getStringExtra("book_id") // storing required book id we get from intent which was fed this info from dashboard fragment adapte //
        } else {
            finish()
        ...
        val jsonParams =
            JSONObject() // this will go with url (this is json object holding bookid of the book we want info for)
        jsonParams.put(
            "book_id",
            bookId
        ) // in variable book_id we store the value of bookID we got from Dashboardfragment as intent.
                ...
                Response.Listener {
                    try {
                        ...
                        if (success) {
                            ...
                            val bookjsonObject =
                                it.getJSONObject("book_data") //book_data is the key in which json info is sent by api
                            ...
                            val bookEntity =
                                BookEntity( // now creating all the properties of BookEntity class which are required in bookEntity object
                            ...
                            val checkFav = DBAsyncTask(
                                ...
                                bookEntity,
                                1
                            ).execute() // .execute() to start bgProcess
    ...
    class DBAsyncTask(val context: Context, val bookEntity: BookEntity, val mode: Int) :
        AsyncTask<Void, Void, Boolean>() { // 3 parameters of ASYNCTASK CLASS: PARAMS, PROGRESS, RESULT
            ...
            when (mode) { //to perform these operations, we can use the bookDao interface.
                ...
                */
                1 -> { //check db if book is in fav or not
                    val book: BookEntity? = db.bookDao()
                        .getBookById(bookEntity.book_id.toString() /*id of the book we need to check*/)
                ...
                2 -> { //save book in db as fav
                    db.bookDao()
                        .insertBook(bookEntity) // in the currently opened context, (i.e. the descriptionactivity of the book selected from dashboard frag) insert this book to bookEntity
                    db.close()
                    return true
                ...
                3 -> {//remove book from fav
                    db.bookDao()
                        .deleteBook(bookEntity) // in the currently opened context, (i.e. the descriptionactivity of the book selected from dashboard frag) delete this book from bookEntity
                    db.close()
                    return true
